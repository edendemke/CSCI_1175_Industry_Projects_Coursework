...
(Use table view) Revise Listing 31.14 to add a button to delete the selected row from the table.
 */
...
public class AddNewRowDemo extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        hBox.getChildren().add(btDeleteRow);
        hBox.setAlignment(Pos.CENTER);

        btDeleteRow.setOnAction(e -> {
            int attempt = tableView.getSelectionModel().getSelectedIndex();
            Country countryToRemove = data.get(attempt);

            data.remove(countryToRemove);
        });
