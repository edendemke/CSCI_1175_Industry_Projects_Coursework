...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  public List<Integer> getACycle() {
	  List<Integer> myVertexIndices = new ArrayList<>();
	  for (int i = 0; i < vertices.size(); i++) {
		  myVertexIndices.add(vertices.indexOf(vertices.get(i)));
	  }
	  boolean[] isIndexVisited = new boolean[vertices.size()];
	  int[] parentIndices = new int[vertices.size()];
	  for (int i = 0; i < parentIndices.length; i++) {
		  parentIndices[i] = -1;
	  }
	  
	  while(myVertexIndices.size() > 0) {
		  int vIndex = myVertexIndices.get(0);
		  Stack<Integer> stack = new Stack<>();
		  stack.push(vIndex);
		  isIndexVisited[vIndex] = true;
		  myVertexIndices.remove(new Integer(vIndex));
		  ...
		  while(!stack.isEmpty()) {
			  ...
			  if(neighbors.get(x).size() == 0) {
				  ...
				  for(int i = 0; i < neighbors.get(x).size(); i++) {
					  ...
					  if(!isIndexVisited[e.v]) {
						  parentIndices[e.v] = x;
						  stack.push(e.v);
						  isIndexVisited[e.v] = true;
						  myVertexIndices.remove(new Integer(e.v));
						  neighbors.get(x).remove(i);
						  break;
					  } else if (e.v != parentIndices[x]) {
